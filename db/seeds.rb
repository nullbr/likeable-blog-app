...
def build_ratings(post_id)
  users = User.all.to_a.sample(rand(1..20))
  users.map do |user|
    {
      user_id: user.id,
      post_id: post_id,
...
NUM_POSTS.times.each_slice((NUM_POSTS / THREAD_COUNT).ceil) do |slice|
...
Post.all.limit(post_with_ratings_count).each_slice(post_with_ratings_count / THREAD_COUNT) do |post_slice|
  threads << Thread.new do
    post_slice.each do |post|
      build_ratings(post.id).each { |rating| make_api_request("posts/rate", rating) }
    end
  end
end
